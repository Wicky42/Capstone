...
import "./SellerDashboardStats.css";
...
export default function SellerDashboardStats() {
    ...
    useEffect(() => {
        async function loadStats() {
            try {
                ...
                setError("Die Dashboard-Statistiken konnten nicht geladen werden.");
            } finally {
                setIsLoading(false);
            }
        }
    ...
    if (isLoading) {
        return <div className="seller-dashboard__state">Statistiken werden geladen …</div>;
    }

    if (error) {
        return (
            <div className="seller-dashboard__state seller-dashboard__state--error">{error}</div>
        );
    }
            ...
            {shop && (
                <section className="seller-dashboard__section">
                    ...
                    <div className="seller-dashboard__cards">
                        ...
                        {onboarding && (
                            <div className="seller-dashboard__info-card card">
                                <span className="seller-dashboard__info-label">
                                    Onboarding-Status
                                </span>
                                <span
            ...
            {counts && (
                <section className="seller-dashboard__section">
                    ...
                    <div className="seller-dashboard__stat-grid">
                        <button
                            className="seller-dashboard__stat-card card seller-dashboard__stat-card--clickable"
                        ...
                        <button
                            className="seller-dashboard__stat-card seller-dashboard__stat-card--active card seller-dashboard__stat-card--clickable"
                            onClick={() => navigate("/seller/products?status=ACTIVE")}
                        >
                            <span className="seller-dashboard__stat-value">{counts.active}</span>
                            <span className="seller-dashboard__stat-label">Aktiv</span>
                        </button>
                        <button
                            className="seller-dashboard__stat-card seller-dashboard__stat-card--draft card seller-dashboard__stat-card--clickable"
                            onClick={() => navigate("/seller/products?status=DRAFT")}
                        >
                            <span className="seller-dashboard__stat-value">{counts.draft}</span>
                            <span className="seller-dashboard__stat-label">Entwurf</span>
                        </button>
                        <button
                            className="seller-dashboard__stat-card seller-dashboard__stat-card--inactive card seller-dashboard__stat-card--clickable"
                            onClick={() => navigate("/seller/products?status=INACTIVE")}
                        >
